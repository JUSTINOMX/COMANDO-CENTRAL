...
export default function Sidebar({ 
  ...
  const handleMobileNav = (section: string, projectId?: string, agentName?: string) => {
    onNavigate(section, projectId, agentName);
    if (onCloseMobile) {
      onCloseMobile();
    }
  };
  ...
  return (
    <>
      ...
      <aside 
        className={`fixed inset-y-0 left-0 z-50 flex h-full w-72 max-w-[85vw] flex-col border-r border-border bg-white px-4 py-6 transition-transform duration-300 ease-in-out md:static md:w-64 md:translate-x-0 ${
      ...
      >
        ...
        <div className="mb-6 flex items-center justify-between px-2 shrink-0">
          ...
          {onCloseMobile && (
            <button
              onClick={onCloseMobile}
              className="md:hidden text-text-secondary hover:text-text-primary p-1.5 rounded-lg hover:bg-gray-100"
        ...
        <div className="flex flex-1 flex-col gap-6 overflow-y-auto pr-1">
          ...
          <div className="flex flex-col gap-1 shrink-0">
            ...
            {/* Neuron Connect */}
            <button
              onClick={() => handleMobileNav("neuron")}
            ...
            {/* Commander */}
            <button
              onClick={() => handleMobileNav("commander")}
            ...
            {/* STEVE */}
            <button
              onClick={() => handleMobileNav("agent-chat", undefined, "steve")}
              className={`flex items-center gap-3 rounded-lg px-3 py-1.5 text-sm font-medium transition-all ${
            ...
            {/* 🏛️ Consejo */}
            <button
              onClick={() => handleMobileNav("council")}
              className={`flex items-center gap-3 rounded-lg px-3 py-1.5 text-sm font-medium transition-all ${
                ...
                {projects.length === 0 ? (
                  ...
                  projects.map((p) => {
                    ...
                    return (
                      <button
                        key={p.id}
                        onClick={() => handleMobileNav("project", p.id)}
              ...
              <div className="flex flex-col">
                {mergedAreas.map((area) => (
                  <SidebarAgentArea
                    ...
                    onSelectAgent={(name) => handleMobileNav("agent-chat", undefined, name)}
                  />
                ))}
              </div>
            )}
          ...
          <div className="flex flex-col gap-1 shrink-0 mt-auto pt-4 border-t border-gray-100">
            ...
            {/* AI Agents Catalog */}
            <button
              onClick={() => handleMobileNav("agents")}
              className={`flex items-center gap-3 rounded-lg px-3 py-1.5 text-sm font-medium transition-all ${
            ...
            {/* GitHub Installer */}
            <button
              onClick={() => handleMobileNav("github-installer")}
              className={`flex items-center gap-3 rounded-lg px-3 py-1.5 text-sm font-medium transition-all ${
